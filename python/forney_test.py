...
def calculate_syndromes(received):
    ...
    for i in range(1, 17):  # 修改范围为 1 到 n-k
        ...
        for j in range(0, 255):
            syndrome += received_gf[j] * (alpha ** (i * (j)))  # 修改指数计算alpha ^ (-i *j)
            if (i == 0 and (j % 16) == 15):
...
def chien_search(error_locator_poly):
    sigma = error_locator_poly.coeffs[::-1]
    alpha = GF.primitive_element
    print("ALPHA IN chien serach is: ", alpha)
    errors = []
    
    for i in range(0, n, 1):  # 从 0 到254
        x = alpha**(-i)
        # 使用 Horner 方法进行多项式求值
        error = GF(0)
        for coef in sigma:
...
def forney_algorithm(error_locator_poly, error_positions, syndromes, fcr):
    ...
    for i in range(1, len(error_locator_poly.coeffs), 2):
        if i % 2 == 1:
            new_coeffs[i] = error_locator_poly.coeffs[::-1][i]  #需要始终从i
        else:
            new_coeffs[i] = 0
    ...
    for i in error_positions:
        X_i_inv = alpha ** (-i)  #(255 - (254 - i))
        print(f"value of i is {i} and alpha**(255 - i) {int(X_i_inv):02X}")
...
# 验证纠错是否成功
if all(corrected_data == rs.encode(data)[::-1]):
